...
func (s *httpManipulator) RetrieveMany(mctx *manipulate.Context, dest elemental.ContentIdentifiable) error {
	...
	if response.StatusCode != http.StatusNoContent {
		...
		if err := jsoniter.ConfigCompatibleWithStandardLibrary.NewDecoder(response.Body).Decode(dest); err != nil {
			sp.SetTag("error", true)
			sp.LogFields(log.Error(err))
...
func (s *httpManipulator) Retrieve(mctx *manipulate.Context, objects ...elemental.Identifiable) error {
	...
	for _, object := range objects {
		...
		if response.StatusCode != http.StatusNoContent {
			defer response.Body.Close() // nolint: errcheck
			if err := jsoniter.ConfigCompatibleWithStandardLibrary.NewDecoder(response.Body).Decode(&object); err != nil {
				sp.SetTag("error", true)
				sp.LogFields(log.Error(err))
				return manipulate.NewErrCannotUnmarshal(err.Error())
			}
...
func (s *httpManipulator) Create(mctx *manipulate.Context, objects ...elemental.Identifiable) error {
	...
	for _, child := range objects {
		...
		buffer := &bytes.Buffer{}
		if err = jsoniter.ConfigCompatibleWithStandardLibrary.NewEncoder(buffer).Encode(&child); err != nil {
			sp.SetTag("error", true)
			sp.LogFields(log.Error(err))
		...
		if response.StatusCode != http.StatusNoContent {
			defer response.Body.Close() // nolint: errcheck
			if err := jsoniter.ConfigCompatibleWithStandardLibrary.NewDecoder(response.Body).Decode(&child); err != nil {
				sp.SetTag("error", true)
				sp.LogFields(log.Error(err))
				return manipulate.NewErrCannotUnmarshal(err.Error())
			}
...
func (s *httpManipulator) Update(mctx *manipulate.Context, objects ...elemental.Identifiable) error {
	...
	for _, object := range objects {
		...
		buffer := &bytes.Buffer{}
		if err = jsoniter.ConfigCompatibleWithStandardLibrary.NewEncoder(buffer).Encode(object); err != nil {
			sp.SetTag("error", true)
			sp.LogFields(log.Error(err))
			return manipulate.NewErrCannotMarshal(err.Error())
		}
...
func (s *httpManipulator) send(mctx *manipulate.Context, request *http.Request) (*http.Response, error) {
	...
	if response.StatusCode < 200 || response.StatusCode >= 300 {
		...
		defer response.Body.Close() // nolint: errcheck
		if err := jsoniter.ConfigCompatibleWithStandardLibrary.NewDecoder(response.Body).Decode(&es); err != nil {
