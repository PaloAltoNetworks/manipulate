...
func init() {
	apomock.RegisterInternalType("github.com/aporeto-inc/gocql", ApomockStructInflightCachedEntry, apomockNewStructInflightCachedEntry)
	apomock.RegisterInternalType("github.com/aporeto-inc/gocql", ApomockStructNextIter, apomockNewStructNextIter)
	apomock.RegisterInternalType("github.com/aporeto-inc/gocql", ApomockStructTraceWriter, apomockNewStructTraceWriter)
	apomock.RegisterInternalType("github.com/aporeto-inc/gocql", ApomockStructRoutingKeyInfo, apomockNewStructRoutingKeyInfo)
	apomock.RegisterInternalType("github.com/aporeto-inc/gocql", ApomockStructRoutingKeyInfoLRU, apomockNewStructRoutingKeyInfoLRU)
	...
	apomock.RegisterFunc("gocql", "gocql.Query.reset", (*Query).AuxMockreset)
	...
	apomock.RegisterFunc("gocql", "gocql.Query.Release", (*Query).AuxMockRelease)
...
const (
	ApomockStructInflightCachedEntry = 3
	ApomockStructNextIter            = 4
	ApomockStructTraceWriter         = 5
	ApomockStructRoutingKeyInfo      = 6
	ApomockStructRoutingKeyInfoLRU   = 7
)
...
var (
	...
	ErrUseStmt       = errors.New("use statements aren't supported. Please see https://github.com/aporeto-inc/gocql for explaination.")
...
//
// Mock: (recvq *Query)reset()()
//

type MockArgsTypeQueryreset struct {
	ApomockCallNumber int
}

var LastMockArgsQueryreset MockArgsTypeQueryreset

// (recvq *Query)AuxMockreset()() - Generated mock function
func (recvq *Query) AuxMockreset() {
	return
}

// RecorderAuxMockPtrQueryreset  - Stats Recorder to keep a count on how many times the function has been called
var recorderAuxMockPtrQueryreset int = 0

var condRecorderAuxMockPtrQueryreset *sync.Cond = &sync.Cond{L: &sync.Mutex{}}

func AuxMockWaitForRecorderAuxMockPtrQueryreset(i int) {
	condRecorderAuxMockPtrQueryreset.L.Lock()
	for recorderAuxMockPtrQueryreset < i {
		condRecorderAuxMockPtrQueryreset.Wait()
	}
	condRecorderAuxMockPtrQueryreset.L.Unlock()
}

func AuxMockIncrementRecorderAuxMockPtrQueryreset() {
	condRecorderAuxMockPtrQueryreset.L.Lock()
	recorderAuxMockPtrQueryreset++
	condRecorderAuxMockPtrQueryreset.L.Unlock()
	condRecorderAuxMockPtrQueryreset.Broadcast()
}
func AuxMockGetRecorderAuxMockPtrQueryreset() (ret int) {
	condRecorderAuxMockPtrQueryreset.L.Lock()
	ret = recorderAuxMockPtrQueryreset
	condRecorderAuxMockPtrQueryreset.L.Unlock()
	return
}

// (recvq *Query)reset - Mocked version of actual API. Does Recordkeeping and calls the user provided mock or default mock
func (recvq *Query) reset() {
	FuncAuxMockPtrQueryreset, ok := apomock.GetRegisteredFunc("gocql.Query.reset")
	if ok {
		FuncAuxMockPtrQueryreset.(func(recvq *Query))(recvq)
	} else {
		panic("FuncAuxMockPtrQueryreset ")
	}
	AuxMockIncrementRecorderAuxMockPtrQueryreset()
	return
}
...
//
// Mock: (recvq *Query)Release()()
//

type MockArgsTypeQueryRelease struct {
	ApomockCallNumber int
}

var LastMockArgsQueryRelease MockArgsTypeQueryRelease

// (recvq *Query)AuxMockRelease()() - Generated mock function
func (recvq *Query) AuxMockRelease() {
	return
}

// RecorderAuxMockPtrQueryRelease  - Stats Recorder to keep a count on how many times the function has been called
var recorderAuxMockPtrQueryRelease int = 0

var condRecorderAuxMockPtrQueryRelease *sync.Cond = &sync.Cond{L: &sync.Mutex{}}

func AuxMockWaitForRecorderAuxMockPtrQueryRelease(i int) {
	condRecorderAuxMockPtrQueryRelease.L.Lock()
	for recorderAuxMockPtrQueryRelease < i {
		condRecorderAuxMockPtrQueryRelease.Wait()
	}
	condRecorderAuxMockPtrQueryRelease.L.Unlock()
}

func AuxMockIncrementRecorderAuxMockPtrQueryRelease() {
	condRecorderAuxMockPtrQueryRelease.L.Lock()
	recorderAuxMockPtrQueryRelease++
	condRecorderAuxMockPtrQueryRelease.L.Unlock()
	condRecorderAuxMockPtrQueryRelease.Broadcast()
}
func AuxMockGetRecorderAuxMockPtrQueryRelease() (ret int) {
	condRecorderAuxMockPtrQueryRelease.L.Lock()
	ret = recorderAuxMockPtrQueryRelease
	condRecorderAuxMockPtrQueryRelease.L.Unlock()
	return
}

// (recvq *Query)Release - Mocked version of actual API. Does Recordkeeping and calls the user provided mock or default mock
func (recvq *Query) Release() {
	FuncAuxMockPtrQueryRelease, ok := apomock.GetRegisteredFunc("gocql.Query.Release")
	if ok {
		FuncAuxMockPtrQueryRelease.(func(recvq *Query))(recvq)
	} else {
		panic("FuncAuxMockPtrQueryRelease ")
	}
	AuxMockIncrementRecorderAuxMockPtrQueryRelease()
	return
}
