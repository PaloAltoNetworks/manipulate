...
install_mock:
	...
	@$(foreach dir,$(DIRS_WITH_MAKEFILES), pushd $(dir) && if [ ! -d vendor ]; then mkdir vendor; fi && popd;)
	@$(foreach dir,$(DIRS_WITH_MAKEFILES), pushd $(dir) && if [ -d $(APOMOCK) ]; then cp -r $(APOMOCK)/* vendor; fi && popd;)
